...
export class WebAudioFontChannel {
    ...
    bandEqualizer = function (from, frequency) {
...
export class WebAudioFontLoader {
    ...
    startLoad = function (audioContext, filePath, variableName) {
    ...
    ;
    decodeAfterLoading = function (audioContext, variableName) {
        const me = this;
    ...
    ;
    waitOrFinish = function (variableName, onFinish) {
        if (window[variableName]) {
            onFinish();
        }
        else {
    ...
    ;
    instrumentInfo = function (n) {
        const key = this.instrumentKeys()[n];
        const p = 1 * parseInt(key.substring(0, 3));
        return {
            variable: '_tone_' + key,
    ...
    ;
    findInstrument = function (program) {
    ...
    ;
    drumInfo = function (n) {
        const key = this.drumKeys()[n];
        const p = 1 * parseInt(key.substring(0, 2));
        return {
            variable: '_drum_' + key,
...
export class WebAudioFontPlayer {
    ...
    createChannel = function (audioContext) {
        return new WebAudioFontChannel(audioContext);
    };
    ;
    createReverberator = function (audioContext) {
        return new WebAudioFontReverberator(audioContext);
    };
    ;
    limitVolume = function (volume) {
        if (volume) {
            volume = 1.0 * volume;
        }
        else {
    ...
    ;
    queueChord = function (audioContext, target, preset, when, pitches, duration, volume, slides) {
    ...
    ;
    queueStrumUp = function (audioContext, target, preset, when, pitches, duration, volume, slides) {
    ...
    ;
    queueStrumDown = function (audioContext, target, preset, when, pitches, duration, volume, slides) {
        pitches.sort(function (a, b) {
            return a - b;
        });
        return this.queueStrum(audioContext, target, preset, when, pitches, duration, volume, slides);
    ...
    ;
    queueStrum = function (audioContext, target, preset, when, pitches, duration, volume, slides) {
        volume = this.limitVolume(volume);
        ...
        for (let i = 0; i < pitches.length; i++) {
            ...
            const envlp = this.queueWaveTable(audioContext, target, preset, when + i * 0.01, pitches[i], duration, volume - Math.random() * 0.01, singleSlide);
            if (envlp)
                envelopes.push(envlp);
            volume = 0.9 * volume;
        }
    ...
    ;
    queueSnap = function (audioContext, target, preset, when, pitches, duration, volume, slides) {
        volume = this.limitVolume(volume);
    ...
    ;
    resumeContext = function (audioContext) {
        try {
    ...
    queueWaveTable = function (audioContext, target, preset, when, pitch, duration, volume, slides) {
    ...
    ;
    setupEnvelope = function (audioContext, envelope, zone, volume, when, sampleDuration, noteDuration) {
    ...
    ;
    numValue = function (aValue, defValue) {
    ...
    ;
    findEnvelope = function (audioContext, target) {
        let envelope = null;
    ...
    ;
    findZone = function (audioContext, preset, pitch) {
        let zone = null;
    ...
    ;
    cancelQueue = function (audioContext) {
        for (let i = 0; i < this.envelopes.length; i++) {
            const e = this.envelopes[i];
...
export class WebAudioFontReverberator {
    constructor (audioContext) {
        ...
        this.audioContext.decodeAudioData(this.irrArrayBuffer, function (audioBuffer) {
            const c = audioContext.createConvolver();
...
export class WebAudioFontTicker {
    ...
    playLoop = function (player, audioContext, loopStart, loopPosition, loopEnd, queue) {
        this.startTicks(audioContext, function (when, from, to) {
            for (let i = 0; i < queue.length; i++) {
                const note = queue[i];
                if (note.when >= from && note.when < to) {
                    const start = when + note.when - from;
    ...
    ;
    startTicks = function (audioContext, onTick, loopStart, loopPosition, loopEnd, onEnd) {
    ...
    ;
    tick = function (audioContext, nextAudioTime, onTick, loopStart, loopPosition, loopEnd, onEnd) {
