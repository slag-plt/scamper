...
export class WebAudioFontPlayer {
  ...
  queueChord = function (
    ...
    for (let i = 0; i < pitches.length; i++) {
      ...
      const envlp = this.queueWaveTable(
        audioContext,
        target,
        preset,
        when,
    ...
    if (zone) {
        ...
        if (slides.length > 0) {
          ...
          for (let i = 0; i < slides.length; i++) {
            ...
            const newPlaybackRate =
              1.0 * Math.pow(2, (100.0 * nextPitch - baseDetune) / 1200.0)
      ...
      if (loop) {
        ...
        envelope.audioBufferSourceNode.loopStart =
          zone.loopStart / zone.sampleRate + (zone.delay ? zone.delay : 0)
        envelope.audioBufferSourceNode.loopEnd =
          zone.loopEnd / zone.sampleRate + (zone.delay ? zone.delay : 0)
      } else {
