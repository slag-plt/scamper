...
import { Range, mkRange } from '../range.js'
...
export function parseSingle (t: Token, wildAllowed: boolean): V.Syntax {
  const text = t.text
  if (intRegex.test(text)) {
    return V.mkSyntax(t.range, parseInt(text))
  } else if (floatRegex.test(text)) {
    return V.mkSyntax(t.range, parseFloat(text))
  } else if (text === '#t') {
    return V.mkSyntax(t.range, true)
  } else if (text === '#f') {
    return V.mkSyntax(t.range, false)
  } else if (text === 'null') {
    return V.mkSyntax(t.range, null)
  } else if (text.startsWith('"')) {
    return V.mkSyntax(t.range, parseStringLiteral(text, t.range))
  } else if (text.startsWith('#\\')) {
    const escapedChar = text.slice(2)
    if (escapedChar.length === 1) {
      return V.mkSyntax(t.range, V.mkChar(escapedChar))
    } else if (namedCharValues.has(escapedChar)) {
      return V.mkSyntax(t.range, V.mkChar(namedCharValues.get(escapedChar)!))
    } else {
    ...
    return V.mkSyntax(t.range, V.mkSymbol(text))
  }
}

export function parseValue (tokens: Token[]): V.Syntax {
  ...
  if (isOpeningBracket(beg.text)) {
    ...
    if (tokens.length === 0) {
      ...
      return V.mkSyntax(
        ...
        beg.text === '[' ? values : V.mkList(...values))
    }
  } else if (beg.text === "'") {
    return V.mkSyntax(beg.range, V.mkList(V.mkSymbol('quote'), parseValue(tokens)))
  } else {
    return parseSingle(beg, true)
  }
}

export function parseValues (tokens: Token[]): V.Syntax[] {
  const ret = []
