...
export interface Ctor extends Node {
  tag: "ctor"
  name: string
  fields: string[]
}
export interface Cls extends Node {
  tag: "cls"
  params: string[]
  body: Blk
  name?: string
}
export interface Ap extends Node {
  tag: "ap"
  numArgs: number
}
...
export interface Disp extends Node {
  tag: "disp"
  expr: Blk
}
...
export interface StmtExp extends Node {
  tag: "stmtexp"
  expr: Blk
}
...
export interface PWild extends Node {
  tag: "pwild"
}
...
export interface PCtor extends Node {
  tag: "pctor"
  name: string
  args: Pat[]
}
...
export function progToNode(prog: Prog): ProgNode {
  // get entire range of the program
  const range = Range.none
  for (const stmt of prog) {
    if (stmt.range.end > range.end) range.end = stmt.range.end
  }
