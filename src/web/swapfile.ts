...
export function saveSwapFile (fs: FS.t, filename: string, swap: SwapData) {
  // Debug log removed
...
export async function loadSwapFile (fs: FS.t, filename: string): Promise<SwapData | undefined> {
  if (!filename.startsWith('.')) {
    const swapFilename = computeSwapFilename(filename)
...
export async function deleteSwapFile (fs: FS.t, filename: string) {
  if (!filename.startsWith('.')) {
    const swapFilename = computeSwapFilename(filename)
    if (await fs.fileExists(swapFilename)) {
