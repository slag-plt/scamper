...
export class FS {
  ...
  async init(): Promise<void> {
    const localStorageFilesExist = JSON.parse(localStorage.getItem(this.fileListTag)!) !== null
  ...
  async migrateLocalStorage (): Promise<void> {
    const fileList = JSON.parse(localStorage.getItem(this.fileListTag)!) as string[]
  ...
  async loadFile (filename: string, lock?: boolean): Promise<string> {
    let workerResult = await this.messageWorker({ type: 'ReadFile', path: filename, lock })
  ...
  async saveFile (filename: string, content: string, lock?: boolean): Promise<void> {
    const directoryHandle = await navigator.storage.getDirectory()
    await directoryHandle.getFileHandle(filename, { create: true })
    let workerResult = await this.messageWorker({ type: 'WriteFile', path: filename, content, lock })
  ...
  async renameFile (from: string, to: string) {
    let workerResult = await this.messageWorker({ type: 'MoveFile', source: from, destination: to })
    if (workerResult.type === 'Error') {
      throw new Error(workerResult.message)
    }
  }
  ...
  async makeUntitledFile (): Promise<string> {
    ...
    if (!exists) {
      ...
      let i = 0
      while (await this.fileExists(`${base}-${i}.scm`)) { i++ }
