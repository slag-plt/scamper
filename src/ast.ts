...
class SyntaxNode {
  ...
  constructor(
    ...
    switch (typeof v) {
      case "boolean":
        this.value = "Boolean " + v.toString();
        this.name = "a boolean " + v.toString();
        this.simplename = v.toString();
        break;
      case "number":
        this.value = "Numerical " + v.toString();
        this.name = "the number " + v.toString();
        this.simplename = v.toString();
        break;
      case "string":
        this.value = '"' + v + '"';
        ...
        if (LPM.isPair(v)) {
          ...
          while (LPM.isPair(tail)) {
            const child = new SyntaxNode(
              S.mkSyntax(
                (tail.fst as S.Syntax).value,
                (tail.fst as S.Syntax).range,
              ),
          ...
          if (this.parentname != null) {
            this.name =
              "the parenthesis list in argument " +
              this.index +
              " of " +
              this.parentname;
          } else {
            this.name =
              "the parenthesis starting with " + this.children[0].name;
          }
          ...
          this.name =
            "the square bracket array at " + this.syntax.range.toString();

          if (this.parentname != null) {
            this.name =
              "the square bracket array in argument " +
              this.index +
              " of " +
              this.parentname;
          }
          ...
          for (const c of v as LPM.Vector) {
            const child = new SyntaxNode(
              S.mkSyntax((c as S.Syntax).value, (c as S.Syntax).range),
              this.name,
              i,
            );
            child.parent = this;
          ...
          if (this.parentname == null) {
            this.name =
              "the square bracket array starting with " + this.children[0].name;
          }
        } else if (LPM.isSym(v)) {
...
export class AST {
  ...
  renderNode(
    ...
    label.setAttribute(
      "aria-label",
      `${node.value}, Level ${level.toString()}, item ${(indexInParent + 1).toString()} of ${totalSiblings.toString()}`,
    );

    label.textContent = node.value;

    label.onclick = () => {
      console.log("Clicked " + node.syntax.range.toString());
      ...
      editor.dispatch({
        selection: EditorSelection.create([
          EditorSelection.range(
            node.syntax.range.begin.idx,
            node.syntax.range.end.idx + 1,
          ),
        ]),
      });
    };
    ...
    label.addEventListener("keydown", (e: KeyboardEvent) => {
      if (e.key === "ArrowDown") {
        ...
        if (
          node.parent &&
          node.index !== null &&
          node.index + 1 < node.parent.children.length
        ) {
  ...
  render(output: HTMLElement, editor: EditorView) {
    ...
    for (let i = 0; i < this.nodes.length; i++) {
      ...
      container.appendChild(
        this.renderNode(node, 1, isLast, editor, i, this.nodes.length),
      );
    }
