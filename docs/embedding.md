...
There are two ways to embed Scamper code.
The above example puts the code in the element itself.
Alternatively, we can place code in a `<script>`:
...
One widget's button sees its own definitions and reports its own errors, and starting a later widget does not tear down an earlier one's animation.
