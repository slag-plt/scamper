...
Read once before working on either half of the file system; consult it again
when changing the boundary between them.

`docs/server.md` covers running the server and its API;
`docs/server-deployment.md` covers putting it on a host.
