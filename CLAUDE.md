...
+ **`npm run validate` must be clean before a pull request is *opened*.** CI runs exactly it (`npm ci && npm run validate`), and `build (22.x)` is a required check, so a pull request opened red cannot merge. Pushing a branch is free -- the workflow triggers only on a pull request and on main, so a branch with no pull request runs no CI at all. Open one when the work is ready, not to store it
+ `npm run validate` does not start a server. The compose stack -- migrations, account creation, sign-in, saving a file -- is covered only by the `server-smoke` job in CI, which is the one thing that exercises `better-auth migrate`. A change to `server/`, `docker-compose.yml`, or the BetterAuth dependency is not verified until that job is green
