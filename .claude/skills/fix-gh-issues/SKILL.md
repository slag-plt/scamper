...
7. For each issue that you fix:
    + If the fix changes what a user can observe in the app, add one line to the patch notes (`patch-notes.md`) describing it, under the `# next` heading -- creating that heading if it is not there yet. See the Patch notes section of `CLAUDE.md.
